...
def run_parameter_setting4(kwargs):
    ...
    for model_number in range(48):
        ...
        trained_model, _ = build_model_stateful1(kwargs["setting"], X_train, y_train)
        collection_trained_models.append(trained_model)

    all_predictions, all_references = test_set_prediction_sf(kwargs["path_to_array"], collection_trained_models, kwargs["setting"], kwargs["ts"],kwargs["ts"].test_true, True)
    print("Model 4 prediction finished.")
    show_all_forecasts(all_predictions, all_references, "", True, "./outputs")
    outputs_model = dict()
    for method in ["MSE", "RMSE", "NRMSE", "MAE", "MAPE"]:
        output: float = Switcher(method, all_predictions, all_references)
        outputs_model[method] = output
